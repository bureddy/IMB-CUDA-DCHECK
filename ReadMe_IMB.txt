--------------------------------------------------
Intel(R) MPI Benchmarks 4.0 Beta
README
--------------------------------------------------
...
1. Build the Intel MPI Benchmarks for the host system:
   host$ source <path to composerxe_mic \
        directory>/composerxe_mic/bin/compilervars.sh intel64
...
1) Check that the Include, Lib, and Path environment variables are set as follows:
    %I_MPI_ROOT%\em64t\include
    %I_MPI_ROOT%\em64t\lib
    %I_MPI_ROOT%\mpi\em64t\bin
...
C:\Program Files (x86)\Intel\mpi\x.x.xxx, where x.x.xxx is the package number.
2) Open the ".vcproj" or ".vcxproj" file for the component you would like to 
   ...
   c) Check other settings as required, for example:
    ...
    Linker > Input
       - Set "Additional Dependencies" to "$(I_MPI_ROOT)\em64t\lib\impi.lib". 
...
installation directory. The default installation directory is
C:\Program Files (x86)\Intel\mpi\x.x.xxx, where x.x.xxx is the package number.
2)Open the relevant Visual Studio 2008*, Visual Studio 2010*, or Visual 
  Studio 2012* projects under the WINDOWS subfolder for Intel(R) MPI Benchmarks.
